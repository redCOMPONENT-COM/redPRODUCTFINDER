...
?>
<form method="post" name="redPRODUCTFINDERFORM<?php echo $modid?>" action="<?php  echo JRoute::_ ( 'index.php?option=com_redproductfinder&task=findproducts&view=redproductfinder' );?>">
   ...
   if ($pretext)
    ...
    $takedropval= $hide_dropdownvalue;//JRequest::getVar('hide_dropdownvalue');
	$takedropname= $hide_dropdown;//JRequest::getVar('hide_dropdown');
	...
	foreach ($types as $key => $type)
	{
			...
			elseif($type->type_select == "Productfinder datepicker")
			{
				if($type->picker==0)
				{
					...
					echo "From : ".JHTML::_('calendar',$sdate , 'from_startdate_ajax', 'from_startdate_ajax',$format = '%d-%m-%Y',array('class'=>'inputbox', 'size'=>'15',  'maxlength'=>'19')); 
					echo " To : ".JHTML::_('calendar',$edate , 'to_enddate_ajax', 'to_enddate_ajax',$format = '%d-%m-%Y',array('class'=>'inputbox', 'size'=>'15',  'maxlength'=>'19')); 
				}else{
					...
					echo "<select multiple name='month[]' class='multiselect' size='5'>\n";
					for ($i=0; $i <= 1; $i++) 
					{
						foreach ($m as $value) 
						{
					...
					/*else{
						for ($i=0; $i < 1; $i++) 
						{
							foreach ($m as $value) 
							{
								$month_name = date('F',mktime(0, 0, 0, $value));
								$mval=$value."-".$month_name."-".$fullyear;
								if($myarr=='')
								{
								$display=$month_name."-".$fullyear;
